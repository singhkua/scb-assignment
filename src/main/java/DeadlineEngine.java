...
public interface DeadlineEngine {
    /**
     *   * Request a new deadline be added to the engine.  The deadline is in millis offset
     * from
     *   * unix epoch. https://en.wikipedia.org/wiki/Unix_time
     *   * The engine will raise an event whenever a deadline (usually now in millis) supplied
     * in the poll method
    ...
    /**
     *   * Remove the scheduled event using the identifier returned when the deadline was
     * scheduled.
    ...
    /**
     *   * Supplies a deadline in millis to check against scheduled deadlines.  If any
     * deadlines are triggered the
     *   * supplied handler is called with the identifier of the expired deadline.
     *   * To avoid a system flood and manage how many expired events we can handle we also
     * pass in the maximum number of
     *   * expired deadlines to fire.  Those expired deadlines that wern't raised will be
     * available in the next poll.
